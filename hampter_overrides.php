...
class Hampter_Overrides extends Module
{
    ...
    public function hookDisplayHeader($params)
    {
        ...
        $scripts .= '        createOrder: function() {' . PHP_EOL;
        $scripts .= '          var createOrderUrl = sc_init_url + (sc_init_url.indexOf("?") === -1 ? "?" : "&") + "ajax=1&action=CreateOrder";' . PHP_EOL;
        $scripts .= '          var payload = { page: "cart", source_page: "cart", addAddress: 1, sc: true, short_cut: 1 };' . PHP_EOL;
        $scripts .= '          return fetch(createOrderUrl, {' . PHP_EOL;
        $scripts .= '            method: "POST",' . PHP_EOL;
        $scripts .= '            credentials: "same-origin",' . PHP_EOL;
        $scripts .= '            headers: { "Content-Type": "application/json" },' . PHP_EOL;
        $scripts .= '            body: JSON.stringify(payload)' . PHP_EOL;
        $scripts .= '          })' . PHP_EOL;
        $scripts .= '            .then(function(res) { return res.text(); })' . PHP_EOL;
        $scripts .= '            .then(function(text) {' . PHP_EOL;
        $scripts .= '              var data;' . PHP_EOL;
        $scripts .= '              try { data = JSON.parse(text); } catch (e) { console.error("[hampter] ScInit returned non-JSON:", text); throw e; }' . PHP_EOL;
        $scripts .= '              if (!data.success || !data.idOrder) { console.error("[hampter] ScInit error:", data); throw new Error("Order creation failed"); }' . PHP_EOL;
        $scripts .= '              return data.idOrder;' . PHP_EOL;
        $scripts .= '            });' . PHP_EOL;
        $scripts .= '        },' . PHP_EOL;
        $scripts .= '        onApprove: function(data) {' . PHP_EOL;
        $scripts .= '          window.location.href = scOrderUrl + "&token=" + encodeURIComponent(data.orderID) + "&PayerID=" + encodeURIComponent(data.payerID);' . PHP_EOL;
